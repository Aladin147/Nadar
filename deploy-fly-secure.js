...
// Try to find flyctl in common locations
function getFlyCommand() {
  const os = require('os');
  const possiblePaths = [
    'fly',
    'flyctl',
    `${os.homedir()}\\.fly\\bin\\flyctl.exe`,
    'C:\\Users\\BLACKWOODS\\.fly\\bin\\flyctl.exe',
...
function runCommand(command, cwd = process.cwd()) {
  return new Promise((resolve, reject) => {
    // Replace 'fly' with the actual flyctl path
    const actualCommand = command.replace(/^fly\b/, `"${flyCmd}"`);
...
async function deployToFly() {
  ...
  try {
    ...
    try {
      await runCommand('fly auth whoami');
    } catch {
      console.log('🔐 Please authenticate with Fly.io first:');
      console.log('   Run: fly auth login');
      console.log('   Or set your token: fly auth token YOUR_TOKEN');
      throw new Error('Not authenticated with Fly.io');
    }
    ...
    try {
      ...
      console.log('📝 Creating new Fly.io app with generated name...');
      await runCommand('fly apps create --generate-name', serverDir);
    }
...
function updateAppConfig(serverUrl) {
  ...
  const envContent = `# Auto-generated by deploy-fly-secure.js
