...
exports.analyzeTelemetryData = analyzeTelemetryData;
 ...
 */
function estimateImageTokens(imageBytes) {
 ...
 */
function calculateRequestCost(entry) {
    ...
    const geminiInputCost = (inputTokens / 1000000) * pricing.INPUT_PER_1M_TOKENS;
    const geminiOutputCost = (outputTokens / 1000000) * pricing.OUTPUT_PER_1M_TOKENS;
    const geminiImageCost = (imageTokens / 1000000) * pricing.IMAGE_PER_1M_TOKENS;
 ...
 */
function analyzeTelemetryData(logData, daysBack) {
    ...
    for (const engine in engineBreakdown) {
        const engineEntries = filteredEntries.filter(e => (e.engine || e.provider_name || 'unknown') === engine);
 ...
 */
function printCostReport(stats) {
